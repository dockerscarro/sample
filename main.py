...
def load_logs_from_db():
    ...
    conn.close()
    return [
        (f"{row[3].strftime('%Y-%m-%d %H:%M:%S.%f')} UTC [{row[1]}] {row[2]}: {row[0]}", row[2])
        for row in rows
    ]
...
def index_texts(texts, batch_size=500):
    ...
    embeddings = embedder.encode([t[0] for t in texts], show_progress_bar=True)
    ...
    for i in range(0, len(texts), batch_size):
        batch_points = [
            PointStruct(id=ids[j], vector=embeddings[j].tolist(), payload={"text": texts[j][0], "level": texts[j][1]})
            for j in range(i, min(i + batch_size, len(texts)))
        ]
        client.upsert(collection_name=COLLECTION_NAME, points=batch_points)
...
def qdrant_search(query, k=200, sim_threshold=0.15):
    ...
    query_vec = embedder.encode([query])[0]
    embeddings = embedder.encode([t[0] for t in texts], show_progress_bar=False)
    sim_scores = cosine_similarity([query_vec], embeddings).flatten()
...
def extract_log_ts(log):
    try:
        ts_str = log[0].split(" UTC")[0].strip()
        return datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S.%f")
    except ValueError:
        try:
            return datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S")
