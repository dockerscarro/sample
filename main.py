...
def connect_pg():
    return psycopg2.connect(dbname=PG_DB, user=PG_USER, password=
        PG_PASSWORD, host=PG_HOST, port=PG_PORT)
    ...
    for line in text.splitlines():
        ...
        if match:
            ts = datetime.strptime(match.group('timestamp'),
                '%Y-%m-%d %H:%M:%S.%f')
            entries.append((match.group('message').strip(), int(match.group
                ('pid')), match.group('level'), ts))
    return entries
...
def index_texts(texts, batch_size=500):
    ...
    if not isinstance(embeddings, np.ndarray) or embeddings.ndim != 2:
        st.error(
            f"Unexpected embedding shape: {getattr(embeddings, 'shape', 'None')}"
            )
        return
    ...
    for i in range(0, len(texts), batch_size):
        batch_points = [PointStruct(id=ids[j], vector=embeddings[j].tolist(
            ), payload={'text': texts[j]}) for j in range(i, min(i +
...
def build_text_loglevel_clusters_v2(source_collection='hybrid-search25',
    profile_collection='text_loglevel_clusterss', window=100):
    """Fetch logs from Qdrant, group by template+level, build clusters, and insert into profile_collection."""
    import bisect
    from collections import defaultdict
    all_points = []
    points, next_page = client.scroll(collection_name=source_collection,
        with_payload=True, with_vectors=False, limit=1000)
    all_points.extend(points)
    while next_page is not None:
        points, next_page = client.scroll(collection_name=source_collection,
            with_payload=True, with_vectors=False, limit=1000, offset=next_page
            )
        all_points.extend(points)
    time_pattern = re.compile(
        '^(\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d+)\\s+UTC\\s+\\[(\\d+)]\\s+([A-Z]+):\\s+(.+)$'
        )
    messages = []
    for idx, p in enumerate(all_points):
        ...
        try:
            dt_obj = datetime.strptime(dt_str, '%Y-%m-%d %H:%M:%S.%f').replace(
                tzinfo=timezone.utc)
            ts = int(dt_obj.timestamp())
        except Exception:
            continue
        messages.append({'id': f'{p.id}_{ts}_{idx}', 'timestamp': ts,
            'raw_timestamp': dt_str, 'pid': int(pid_str), 'text': text_msg.
            strip(), 'log_level': level.upper()})
    ...
    for msg in messages_sorted:
        template_text = re.sub('\\d+', '<NUM>', msg['text']).strip()
        ...
        for m in msgs:
            ...
            for n in messages_sorted[start_idx:end_idx]:
                if n['id'] not in seen_ids:
                    ...
                    seen_ids.add(n['id'])
        clusters.append({'cluster_id': hash((template_text, log_level)) % 
            10 ** 12, 'anchor_text': template_text, 'log_level': log_level,
            'messages': cluster_messages, 'profiling': {'cluster_size': len
            (cluster_messages)}})
    if client.collection_exists(profile_collection):
        client.delete_collection(profile_collection)
    client.create_collection(collection_name=profile_collection,
        vectors_config=VectorParams(size=embedder.
    ...
    for cluster in clusters:
        ...
        client.upsert(collection_name=profile_collection, points=[
            PointStruct(id=cluster['cluster_id'], vector=vector, payload=
            cluster)])
...
def qdrant_search(query, k=200, sim_threshold=0.15):
    ...
    if target_date and any(phrase in query.lower() for phrase in ['from',
        'since', 'after', 'on', 'logs from', 'show logs from']) and not any(
        kw in query.lower() for kw in ['error', 'fail', 'division', 'crash',
        'panic']):
        date_only_mode = True
...
def cluster_logs(logs, n_clusters=10):
    ...
    clustered_results = [sorted(group, key=extract_log_ts) for group in
        clusters.values()]
...
def query_cluster_profiles(user_query, top_k=10):
    ...
    results = client.search(collection_name='text_loglevel_clusterss',
        query_vector=q_vector, limit=top_k, with_payload=True)
    if not results:
        return []
...
LOG_PATTERN = re.compile(
    '(?P<timestamp>\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d{3}) UTC \\[(?P<pid>\\d+)] (?P<level>[A-Z]+):\\s+(?P<message>.+)'
    )
...
uploaded_file = st.file_uploader('📁 Upload a .txt, .pdf, or .docx file',
    type=['txt', 'pdf', 'docx'])
for log in logs:
    st.markdown(color_log_message(log.level, log.message),
        unsafe_allow_html=True)
if uploaded_file:
    if ('uploaded_file_name' not in st.session_state or st.session_state.
        ...
        with st.spinner('📥 Parsing and inserting logs into database...'):
            ...
            if inserted_count > 0:
                st.success(
                    f'✅ Inserted {inserted_count} new log entries into PostgreSQL.'
                    )
...
if search_clicked and query and st.session_state.get('indexed'):
    with st.spinner('🔍 Searching logs...'):
        ...
        for i, cluster in enumerate(clusters, 1):
            ...
            st.markdown(
                f"**Cluster Size:** {len(cluster.get('messages', []))} messages"
                )
            ...
            cluster_text = ' '.join([m.get('text', '') for m in cluster.get
                ('messages', [])])
            cluster_vector = embedder.encode(cluster_text).tolist()
            similar_clusters = client.search(collection_name=
                'text_loglevel_clusterss', query_vector=cluster_vector,
            ...
            for sim in similar_clusters:
                ...
                if sim_payload.get('cluster_id') == cluster.get('cluster_id'):
                    continue
                st.markdown(
                    f"- **Anchor Text:** {sim_payload.get('anchor_text', 'N/A')}"
                    )
                st.markdown(
                    f"  **Log Level:** {sim_payload.get('log_level', 'N/A')}")
    ...
    """
        try:
            chat_model = ChatOpenAI(temperature=0, model='gpt-4o-mini',
                openai_api_key=OPENAI_API_KEY, max_tokens=1500)
            response = chat_model([HumanMessage(content=
                'You are a PostgreSQL SRE. Analyze clustered logs efficiently, focusing on key clusters and causal logs with precise time-aware explanations.'
